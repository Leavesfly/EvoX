...
| **evox-models** | LLM 模型适配层 | `OpenAILLM`, `QianWenLLM`, `ErnieBotLLM` | ✅ |
| **evox-actions** | Action 动作执行引擎 | `Action`, `PlanAction`, `ReflectionAction` | ✅ |

### 能力层模块 (Capability Layer)

| 模块 | 功能描述 | 主要类/接口 | 状态 |
|------|---------|-----------|------|
| **evox-storage** | 多种存储适配（内存/数据库/向量） | `InMemoryStore`, `VectorStore`, `GraphStore` | ✅ |
| **evox-memory** | 短期和长期记忆管理 | `ShortTermMemory`, `LongTermMemory` | ✅ |
| **evox-tools** | 丰富的工具集成 | `FileSystemTool`, `HttpTool`, `DatabaseTool` | ✅ |
| **evox-utils** | 通用工具类和提示词管理 | `CommonUtils`, `PromptTemplate` | ✅ |

### 业务层模块 (Business Layer)

| 模块 | 功能描述 | 主要类/接口 | 状态 |
|------|---------|-----------|------|
| **evox-agents** | 多种 Agent 实现和管理 | `ActionAgent`, `ReActAgent`, `RouterAgent` | ✅ |
| **evox-frameworks** | 多智能体协同框架 | `DebateFramework`, `ConsensusFramework` | ✅ |
...
| 模块 | 功能描述 | 示例/测试 | 状态 |
|------|---------|---------|------|
| **evox-examples** | 完整示例应用 | `SimpleChatBot`, `ComprehensiveChatBot` | ✅ |
| **evox-benchmark** | 性能基准测试 | `GSM8K`, `HotpotQA`, `HumanEval` | ✅ |
| **evox-spring-boot-starter** | Spring Boot 自动配置 | `EvoXAutoConfiguration` | ✅ |
